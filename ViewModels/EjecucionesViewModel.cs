...
public partial class EjecucionesViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private string _actionVerb = "Ejecutando";
    ...
    public void LoadFromConfig()
    {
        ...
        var source = SelectedCategory != null
            ...
            : cfg.Categorias.Where(c => c.Tipo is "Ejecucion" or "External");
        ...
        foreach (var cat in source)
        {
            foreach (var item in cat.Items)
            {
                var app = new ApplicationItem
                {
                    ...
                    Icono = cat.Tipo == "External" ? "OpenInNew" : "PlayArrow",
    ...
    [RelayCommand]
    private async Task ExecuteAppAsync(ApplicationItem? app)
    {
        if (app == null || app.IsInstalled) return;

        if (app.RequireConfirm)
        {
        ...
        if (app.Variables.Count > 0)
        {
            ...
            for (int i = 0; i < app.Variables.Count; i++)
            {
                ...
                if (input is System.Windows.Controls.PasswordBox pw)
                    variables[app.Variables[i].Name] = pw.Password;
                else if (input is TextBox tb)
                    variables[app.Variables[i].Name] = tb.Text;
            }
        }
        ...
        ProgressMax = 1;
        ProgressValue = 0;
        ProgressStepMax = app.Pasos.Count;
        ProgressStep = 0;
        _sw.Restart();
        ...
        try
        {
            ...
            _sw.Stop();
            _progressTimer.Stop();
            ProgressMessage = $"✅ Completado ({ElapsedDisplay})";
        }
